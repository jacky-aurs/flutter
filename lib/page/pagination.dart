...
import 'package:fultterapp/common/s/mode/StoryModel.dart';
import 'package:fultterapp/page/home/HomeBanner.dart';
import 'package:url_launcher/url_launcher.dart';

class Pagination extends StatelessWidget {
  static final String routeName = '/page/pagination';
  ...
  void _launchaURL(String url) async {
  ...
  List<Widget> _pageSelector(BuildContext context) {
    List<Widget> list = [];
    List<StoryModel> bannerStores = [];
    arr.forEach((item) {
      bannerStores.add(StoryModel.fromJson(item));
    });

    if (arr.length > 0) {
      list.add(HomeBanner(bannerStores, (stoty) {
        _launchaURL('${stoty.url}');
      }));
    }
    return list;
  }
